<%-- 
    Document   : finalstudlogin
    Created on : 16 Jul, 2019, 10:55:26 PM
    Author     : dinesh
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link rel="stylesheet" href="css/style.css" type="text/css">
		<title>COLMARS ACADEMY</title>
    </head>
    <body>
        <img src="logo1.jpg" height="50" width="50" style="float:left;margin:0;padding:20px 0 0 150px;"> 
		<div id="container">
			<div id="header">

				<h1><span class="blue-text">COLMARS</span> ACADEMY</h1>
				<h2>TAKE YOUR FIRST LEAP<span class="white-text">TOWARDS SUCCESS</span></h2>
			</div><!--header end-->
			<div id="menu">
				<ul>
					 <li class="menu item"><a href="home.jsp">Home</a></li>
					 <li class="menu item"><a href="aboutus.jsp">About Us</a></li>
					 <li class="menu item"><a href="contactus.jsp">Contact Us</a></li>
				</ul>
			</div><!--menu end-->
			<div id="leftmenu">
				<h3>Links</h3>
				<ul>
					 <li ><a href="finalstudlogin.jsp">STUDENT LOGIN</a></li>
					 <li ><a href="facullogin.jsp">FACULTY LOGIN</a></li>
					 <li ><a href="adminlogin.jsp">ADMIN LOGIN</a></li>
					 <li ><a href="celebratedevents.jsp">CELEBRATED EVENTS</a></li>
					 <li ><a href="upcomingevents.jsp">UPCOMING EVENTS</a></li>
					 <li ><a href="feesstructure.jsp">FEES STRUCTURE</a></li>

				</ul>
			</div><!--leftmenu end-->
		</div><!--containers ends-->
		<div id="content">
			<form class="box" action="studvalidate.jsp">
<h3>LOGIN</h3>
<input type="text" required="" name="username" placeholder="USERNAME"><br><br>
<input type="password" name="password" required="" placeholder="PASSWORD"><br><br>
<center><input type="submit" value="LOGIN"><a href="pass.jsp">   Change Password</a></center><br><p>No Account,<a href="studreg.jsp">Create One</a></p>

</form>
		</div>

    </body>
</html>
